...
bool show_qx_result(Data **r_data, Data **s_data,
                        RowData res_open,
                        int r_id, int r_ak, int s_id,
                        int res_ak, int res_sel, int res_sel_a,
                        int rows_1, int rows_2, int rows_3) {
    ...
    if (get_rank() == 0) {
        ...
        {
            //real_count[it->first];
            std::cout<<res_count[it->first]<<std::endl;
        }
...
void executeAndTestQuery(int argc, char *argv[]) {
    ...
    std::string query_file = "../vs_fedquery/qx.txt", // Query file
    schema_file = "../vs_fedquery/qx_schema.txt"; // Schema file
    ...
    createDBandQuery(rows, 2, query_file, schema_file, "Qx", query, "Qx_DB", db);
    ...
    // show_qx_result(r_data[0], r_data[1], res_open,
    //                 r_id, r_ak, s_id,
    //                 res_ak, res_sel, res_sel_a,
    //                 rows[0], rows[1], out_size);
